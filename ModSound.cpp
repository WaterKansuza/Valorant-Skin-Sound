...
const std::vector<std::string> SKINS = {
    "Aquarium", "Hellfire", "HypeDragon", "Ashen", "Cyberpunk"
};
...
static void SleepUntil(std::chrono::steady_clock::time_point tp, bool interruptible) {
    for (;;) {
        ...
        if (now >= tp) return;
        if (interruptible && !isShooting) return;
        auto rem = tp - now;
...
static void InitUI() {
    hideCursor();
    clearScreen();
    gotoXy(1, 1); printf("--- VALORANT VANDAL SKIN SOUND ---\n");
    gotoXy(2, 1); printf("Skin Name :");
    gotoXy(3, 1); printf("Volume    :");
    gotoXy(5, 1); printf("F1: Toggle weapon   F3: Exit\n");
    gotoXy(6, 1); printf("Left/Right: Volume -/+5\n");
    gotoXy(7, 1); printf("Up/Down: Change Skin\n");
    fflush(stdout);
}
...
LRESULT CALLBACK KeyboardHookProc(int nCode, WPARAM wParam, LPARAM lParam) {
    if (nCode >= 0 && wParam == WM_KEYDOWN) {
        ...
        case VK_F3:
            ...
            break;
        case VK_RIGHT:
            if (currentVolume < 100) { currentVolume += 5; SetVolume(currentVolume); UpdateVolumeUI(); }
            break;
        case VK_LEFT:
            if (currentVolume > 0)   { currentVolume -= 5; SetVolume(currentVolume); UpdateVolumeUI(); }
            break;
        case VK_UP:
            if (--currentIndex < 0) currentIndex = (int)SKINS.size() - 1;
            UpdateSkinUI();
            break;
        case VK_DOWN:
            if (++currentIndex >= (int)SKINS.size()) currentIndex = 0;
            UpdateSkinUI();
            break;
        case '1':
            isWeaponEquipped = true;
            break;
        case 'Q': case 'E': case 'C': case '2': case '3':
